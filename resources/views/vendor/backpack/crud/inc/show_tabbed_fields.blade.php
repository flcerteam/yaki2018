        ...
        <ul class="nav {{ $horizontalTabs ? 'nav-tabs' : 'nav-stacked nav-pills'}}" role="tablist">
            @foreach ($crud->getTabs() as $k => $tab)
                <li role="presentation" class="{{$k == 0 ? 'active' : ''}}">
                    ...
                    $pos = strpos($tab, "|@|");
                    @endphp
                    <a href="#tab_{{ $pos != 0 ? substr($tab, $pos + 3) : $slug }}" aria-controls="tab_{{ $pos != 0 ? substr($tab, $pos + 3) : $slug }}" role="tab" data-toggle="tab">{{ $pos != 0 ? substr($tab, 0, $pos) : $tab }}</a>
                </li>
            @endforeach
        </ul>
    </div>
...
<div class="tab-content {{$horizontalTabs ? 'col-md-12' : 'col-md-9 m-t-10'}}">
    ...
    @foreach ($crud->getTabs() as $k => $tab)
        @php
            ...
            $pos = strpos($tab, "|@|");
        @endphp
        <div role="tabpanel" class="tab-pane{{$k == 0 ? ' active' : ''}}" id="tab_{{ $pos != 0 ? substr($tab, $pos + 3) : $slug }}">
